...
// this function returns the head in the list.
...
// this function returns the data in the next node.
...
// this function returns the value in the node
...
void node_remove(struct list* lst, void* value){
  ...
  // use a pointer to pointer which helps keep track of the current node's address
...
// goes through the list until the end is reached
  while(*indirect != NULL){
      ...
      // if the current node's value equals the test value, remove the node.
      if((* indirect)->val == value){

        // stores the current node temporily for later memory dellocation.
        struct node *temp = *indirect;

        // this updates the previous node 'next' pointer to bypass the current node.
        *indirect = (*indirect)->next;

        // free the memory of the remove node
        free(temp);
        return;
      }
    indirect = &(*indirect)->next;
